...
export default function ApplicationOfficerReports() {
    ...
    const fetchReport = async () => {
        try {
            setLoading(true);
            setError("");
            const res = await api.get("/officer/reports/applications");
            setReport(res.data.report);
        } catch (err) {
            console.error("Failed to fetch application report:", err);
            setError(err.response?.data?.message || "Failed to load application report");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', `application-report-${new Date().toISOString().split('T')[0]}.pdf`);
            ...
            <MainLayouts>
                <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                    <div className="text-center">
                        ...
                        <p className="text-muted">Loading application report...</p>
                    </div>
                </div>
            </MainLayouts>
        );
